...
Git history should be kept semi-linear. Ideally, branches should be rebased before being merged. A simple merge commit is also accepted, as long as branches do not intertwine (i.e. merging A $->$ B and B $->$ A multiple times).
...
If the branch is linked to an existing issue or task, the branch name should contain its reference. For example, `feat/issue-42-path-simplification`.
...
If a pull request is related to an existing issue or task, its title should contain its reference. For example, `#42 Path geometry simplification`.
